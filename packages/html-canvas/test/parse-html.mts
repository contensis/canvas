...
describe('parseHtml function tests', () => {
  ...
  describe(`Scenario: Parse simple HTML with a specific field configuration`, () => {
    ...
    before(async () => {
      ...
      const canvas = await parseHtml({ field, html });
  ...
  describe(`Scenario: Parse with a connected Contensis client resolving a specific field in a content type`, () => {
    ...
    before(async () => {
      mock = mocks(`parseHtml_client_contentTypeId_fieldId`);
      const client = new Client(clientConfig);
      canvas = await parseHtml({ client, contentTypeId: 'document', fieldId: 'canvas', html });
      block = canvas[0] as HeadingBlock;
    });
